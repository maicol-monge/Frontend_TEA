...
const Registrar = () => {
  ...
  const openCamera = async () => {
    ...
    if (!window.isSecureContext && !isLocalhost) {
      setCameraError(
        "La cámara requiere ejecutar el sitio en HTTPS o en http://localhost."
      );
      return;
    }
  ...
  const startStream = async (facing = "user") => {
    try {
      ...
      setCameraError(
        "No se pudo acceder a la cámara. Use HTTPS o localhost y verifique permisos."
      );
    }
  };
  ...
  const capturePhoto = async () => {
    ...
    if (!video.videoWidth || !video.videoHeight) {
      Swal.fire({
        title: "Error",
        text: "El video no está listo para capturar.",
        icon: "error",
        confirmButtonText: "Aceptar",
      });
      return;
    ...
    Swal.fire({
      ...
      text: "Se usará como imagen de perfil.",
      icon: "success",
      timer: 1500,
  ...
  const handleResponsableChange = (index, field, value) => {
    setResponsablesLegales((prev) => {
      ...
      // Sanitizar nombres/apellidos de responsables: solo letras y espacios
      if (field === "nombre" || field === "apellido") {
        try {
          newVal = value.replace(/[^^\p{L}\s]/gu, "");
        } catch {
          newVal = value.replace(/[^A-Za-zÀ-ÿ\s]/g, "");
        }
      }
  ...
  const responsablesValidos = () => {
    ...
    return responsablesLegales.every((r) => {
      const parOk =
        r.parentesco.trim() !== "" &&
        (r.parentesco !== "OTRO" || (r.parentesco_otro || "").trim() !== "");
      const idOk = r.num_identificacion.trim().length === 10; // ########-#
      return (
        r.nombre.trim() !== "" && r.apellido.trim() !== "" && idOk && parOk
      );
    });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    // Validar que nombres/apellidos solo tengan letras y espacios
    const isNameValid = (s) => {
    ...
    if (!isNameValid(formData.nombres)) {
      Swal.fire({
        title: "Error",
        text: "Nombres: sólo se permiten letras y espacios.",
        icon: "warning",
        confirmButtonText: "Aceptar",
      });
      return;
    ...
    if (!isNameValid(formData.apellidos)) {
      Swal.fire({
        title: "Error",
        text: "Apellidos: sólo se permiten letras y espacios.",
        icon: "warning",
        confirmButtonText: "Aceptar",
      });
      return;
    ...
    // Validar responsables si aplica
    if (Number(formData.privilegio) === 1) {
      for (let i = 0; i < responsablesLegales.length; i++) {
        const r = responsablesLegales[i];
        if (!isNameValid(r.nombre)) {
          Swal.fire({
            title: "Error",
            text: `Responsable ${
              i + 1
            }: nombre inválido (sólo letras y espacios).`,
            icon: "warning",
            confirmButtonText: "Aceptar",
          });
          return;
        }
        if (!isNameValid(r.apellido)) {
          Swal.fire({
            title: "Error",
            text: `Responsable ${
              i + 1
            }: apellido inválido (sólo letras y espacios).`,
            icon: "warning",
            confirmButtonText: "Aceptar",
          });
          return;
    ...
    try {
      ...
      Swal.fire({
        ...
        navigate(
          Number(formData.privilegio) === 0 ? "/pacientes" : "/home_espe"
        );
      });
    } catch (err) {
      Swal.fire({
          ...
          >
            ...
            <div className="card-body">
              <form onSubmit={handleSubmit}>
                <div className="mb-3">
                  <label className="form-label" style={{ color: COLOR_DARK }}>
                    Nombres:
                  </label>
                  <input
                    type="text"
                    ...
                    <div className="mb-3">
                            ...
                            <div className="col-md-12">
                              <div className="form-check">
                                <input
                                  ...
                                  onChange={(e) =>
                                    toggleLinkMainEmail(idx, e.target.checked)
                                  }
                                />
                                <label
                                  className="form-check-label"
                                  htmlFor={`linkEmail-${idx}`}
                                >
                                  Usar el correo del usuario
                                </label>
                              </div>
                              <small className="text-muted">
                                Solo un responsable puede usar el mismo correo
                                del usuario.
                              </small>
                            </div>
                          </div>
                      ...
                      {!responsablesValidos() && (
                        <div className="text-danger mt-2">
                          Complete los campos requeridos (*) con el formato
                          indicado.
                        </div>
                      )}
                    </div>
                      ...
                      <div className="d-flex justify-content-end gap-2">
                        <button
                          ...
                          title={`Cambiar a cámara ${
                            cameraFacing === "user" ? "trasera" : "frontal"
                          }`}
                        >
                          Girar cámara
                        </button>
