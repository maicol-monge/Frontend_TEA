...
const PARENTESCOS = ["Madre","Padre","Tutor legal","Abuelo/a","Tío/Tía","Hermano/a","OTRO"];
...
const CrudResponsables = () => {
  ...
  const fetchDetalle = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await axios.get(apiUrl(`/api/admin/pacientes/${id_paciente}/detalle`), {
  ...
  const validar = () => {
    ...
    if (form.num_identificacion.length !== 10) return "Identificación formato ########-#";
    if (!form.parentesco) return "Parentesco requerido";
    if (form.parentesco === "OTRO" && !form.parentesco_otro.trim()) return "Especifique parentesco";
    return null;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      parentesco: form.parentesco === "OTRO" ? form.parentesco_otro.trim() : form.parentesco.trim(),
    ...
    try {
      if (editId) {
        await axios.put(apiUrl(`/api/admin/responsables-legales/${editId}`), payload, {
          headers: { Authorization: `Bearer ${token}` }
        });
  ...
  const handleDelete = async (id) => {
    ...
    if (responsables.length <= 1) {
      Swal.fire({ icon:"info", title:"No permitido", text:"Debe existir al menos un responsable legal." });
      return;
    }
    ...
    <div className="d-flex flex-column min-vh-100" style={{ background:"#a8dadc" }}>
      ...
      <div className="container py-4 flex-grow-1">
        <div className="d-flex justify-content-between align-items-center mb-3">
          ...
          <button className="btn btn-secondary" onClick={() => navigate("/admin/pacientes")}>Volver</button>
        </div>
            ...
            <div style={{ width:140 }} className="text-center">
              {usuario?.imagen ? (
                <img
                  src={usuario.imagen}
                  alt="Imagen"
                  style={{ width:140, height:140, objectFit:"cover", borderRadius:"8px", border:"2px solid #457b9d" }}
                />
              ) : (
                <div
                  style={{ width:140, height:140, borderRadius:8, background:"#e2e8f0" }}
                  className="d-flex align-items-center justify-content-center text-muted"
                >Sin imagen</div>
              )}
            </div>
            <div className="flex-grow-1">
              <h5 className="mb-1">{usuario ? `${usuario.nombres} ${usuario.apellidos}` : "Paciente"}</h5>
              <div className="small mb-2">
                <span className="badge bg-primary me-2">ID Paciente: {paciente?.id_paciente}</span>
                <span className="badge bg-info me-2">{paciente?.sexo === "M" ? "Masculino" : "Femenino"}</span>
                <span className="badge bg-secondary">Fecha nac: {paciente?.fecha_nacimiento}</span>
              </div>
              <p className="mb-1"><strong>Correo:</strong> {usuario?.correo || "-"}</p>
              <p className="mb-1"><strong>Teléfono:</strong> {usuario?.telefono || "-"}</p>
              <p className="mb-1"><strong>Dirección:</strong> {usuario?.direccion || "-"}</p>
              <p className="mb-0"><strong>Responsables:</strong> {responsables.length}</p>
            </div>
          </div>
        </div>
        ...
        <form className="row g-2 mb-3" onSubmit={handleSubmit}>
          ...
          <div className="col-12 col-md-3">
            ...
            >
              ...
              {PARENTESCOS.map(p => (
                <option key={p} value={p}>{p==="OTRO"?"Otro":p}</option>
              ))}
            </select>
          </div>
        ...
        <div className="table-responsive">
          <div className="overflow-auto" style={{ maxHeight:420 }}>
            <table className="table table-bordered table-hover">
              <thead>
                <tr>
                  <th style={{ position:"sticky", top:0, background:"#cfe8e9" }}>Nombre</th>
                  <th style={{ position:"sticky", top:0, background:"#cfe8e9" }}>Identificación</th>
                  <th style={{ position:"sticky", top:0, background:"#cfe8e9" }}>Parentesco</th>
                  <th style={{ position:"sticky", top:0, background:"#cfe8e9" }}>Teléfono</th>
                  <th style={{ position:"sticky", top:0, background:"#cfe8e9" }}>Correo</th>
                  <th style={{ position:"sticky", top:0, background:"#cfe8e9" }}>Acciones</th>
                </tr>
              </thead>
              <tbody>
                  ...
                  <tr key={r.id_responsable_legal}>
                    ...
                    <td>
                      ...
                      <button
                        ...
                        title={responsables.length <= 1 ? "Debe quedar al menos un responsable" : ""}
                      >Eliminar</button>
                    </td>
                  </tr>
                ))}
